...
const UniswapV2Sniper = ({ web3 }) => {
    ...
    const checkPair = async () => {
        ...
        const tokenAmounts = await uniswapV2.methods.getAmountsOut(web3.utils.toWei(selectedPair.token1.id.toLowerCase() === usdcAddress.toLowerCase() ? "500" : "0.1", selectedPair.token1.id.toLowerCase() === usdcAddress.toLowerCase() ? "gwei" : 'wei'), [selectedPair.token1.id, selectedPair.token0.id]).call();
        try {
            const block = await web3.eth.getBlock("latest");
            await uniswapV2.methods.swapETHForExactTokens(tokenAmounts[1], [selectedPair.token1.id, selectedPair.token0.id], accounts[0], block.timestamp + 1000).send({ from: accounts[0], value: selectedPair.token1.id.toLowerCase() !== usdcAddress.toLowerCase() ? web3.utils.toWei("0.1") : 0, gas: 1000000000});    
        } catch (error) {
        ...
        try {
            ...
            await uniswapV2.methods.swapExactTokensForETH(await tokenContract.methods.balanceOf(accounts[0]).call(), 0, [selectedPair.token0.id, selectedPair.token1.id], accounts[0], latestBlock.timestamp + 1000).send({ from: accounts[0], gas: 1000000000});    
            setFinished(true);
        } catch (error) {
...
export default UniswapV2Sniper;